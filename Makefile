...
VERSION ?= latest
IMG ?= ghcr.io/xzzpig/daemon-cronjob-controller:$(VERSION)
...
helm: manifests kustomize helmify set-image
	$(KUSTOMIZE) build config/default | $(HELMIFY) charts/daemon-cronjob

.PHONY: release
release: build-installer helmify set-image
	@echo "Release process completed. Installer and Helm chart are ready in the dist directory."
